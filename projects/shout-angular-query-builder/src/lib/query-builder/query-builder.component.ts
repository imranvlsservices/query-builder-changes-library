  ...
  implements OnInit, OnChanges, ControlValueAccessor, Validator
{
  ...
  public defaultClassNames: QueryBuilderClassNames = {
    ...
    removeIcon: "q-icon q-edit-icon",
    editIcon: "q-icon q-remove-icon",
    addIcon: "q-icon q-add-icon",
    button: "q-button",
    buttonGroup: "q-button-group",
    removeButton: "q-remove-button",
    editButton: "q-edit-button",
  ...
  inputTemplates: QueryList<QueryInputDirective> =
    new QueryList<QueryInputDirective>();
  ...
  getDefaultField(entity: Entity): Field {
    if (!entity) {
      ...
      const entityFields = this.fields.filter(
        (field) => field && field.entity === entity.value
      );
  ...
  getDefaultOperator(field: Field): string {
    if (field && field.defaultOperator !== undefined) {
      return this.getOperatorValue(
        this.getDefaultValue(field.defaultOperator) as Operator
      );
    } else {
  ...
  editRule(rule: Rule): void {
    ...
    // Pre-fill dialog with rule data
    this.selectedField = rule.field;
    this.selectedCondition = rule.operator;
    this.inputValue = rule.value;
    this.isDialogOpen = true;
    // Wait for modal to render, then trigger Bootstrap modal
    setTimeout(() => {
  ...
  onConditionChange() {
    // Additional condition logic if necessary
  }
  ...
  submit(parent?: RuleSet): void {
    ...
    if (this.isEditing && this.editingRule) {
      ...
      this.editingRule.field = this.selectedField;
      this.editingRule.operator = this.selectedCondition;
      this.editingRule.value = this.inputValue;
      ...
      parent.rules = parent.rules.concat([
        {
          ...
          entity: this.config.fields[this.selectedField]?.entity || null,
        },
      ]);
    }
  ...
  getOperatorDisplayName(operator: string, field: string): string {
    ...
    const selectedOperator = operators.find(
      (op) => this.getOperatorValue(op) === operator
    );

    // Return the localized name if it exists, otherwise return the operator as string
    if (
      typeof selectedOperator === "object" &&
      "localized" in selectedOperator
    ) {
      return selectedOperator.localized;
    }

    return selectedOperator ? selectedOperator.toString() : "N/A";
  }

  getFieldName(fieldValue: string, entity: string): string {
